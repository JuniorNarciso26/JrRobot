...
function renderStatus(text){
  ...
  el('audio_state').textContent=!valid?'Firmware antigo/incompativel':next.audio_test_running==='1'?'Teste em andamento':audioSeq>0?'Ultimo teste #'+audioSeq+' - '+(next.audio_last||'?')+' - '+audioBytes+' bytes':'Pronto para teste - MAX98357A';
  ...
  el('audio_msg').textContent=audioSeq>0?'Diagnostico do audio: teste #'+audioSeq+', resultado '+(next.audio_last||'?')+', '+audioBytes+' bytes transmitidos. Se ESP_OK e nao houve som, confira MAX98357A/falante.':'MAX98357A: LRC GPIO47, BCLK GPIO21, DIN GPIO42. O teste usa o volume ja aplicado e envia um unico comando.';
