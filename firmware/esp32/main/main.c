...
static void print_help(void){ printf("\nJrBot Face OLED V2. Comandos:\n neutro feliz triste animado bravo surpreso pensando cetico sono confuso piscando amor brincalhao preocupado cool bateria\n demo status help\n\n"); }
static void terminal_task(void *p){
    uint8_t rx[64]; char cmd[32]={0}; size_t n=0; print_help();
    while(true){ int count=uart_read_bytes(UART_NUM_0,rx,sizeof(rx),pdMS_TO_TICKS(100)); for(int i=0;i<count;i++){ char ch=(char)rx[i]; if(ch=='\r'||ch=='\n'){ if(!n)continue; cmd[n]='\0'; if(!strcmp(cmd,"status")) printf("JR_STATUS v=2 expression=%s demo=%d oled=0x%02X sda=%d scl=%d commands=%lu frames=%lu look=%d,%d\n",expression_name(current_expression),demo_mode?1:0,oled_address,OLED_SDA,OLED_SCL,(unsigned long)command_count,(unsigned long)frame_count,look_dx,look_dy); else if(!strcmp(cmd,"help")||!strcmp(cmd,"ajuda")) print_help(); else if(!strcmp(cmd,"demo")){ demo_mode=!demo_mode; printf("JR_OK demo=%d\n",demo_mode?1:0);} else if(set_expression_from_command(cmd)){ command_count++; printf("JR_OK command=%lu expression=%s\n",(unsigned long)command_count,expression_name(current_expression)); } else printf("JR_ERROR comando_desconhecido=%s\n",cmd); n=0; } else if(n<sizeof(cmd)-1) cmd[n++]=(char)tolower((unsigned char)ch); }}
}
...
void app_main(void){
    ...
    int demo_i=0; ESP_LOGI(TAG,"JrBot Face OLED V2 iniciado em 0x%02X",oled_address);
    while(true){ uint32_t ms=now_ms(); if(demo_mode && ms-last_demo_ms>2500){ last_demo_ms=ms; current_expression=demo[demo_i++%16]; printf("JR_DEMO expression=%s\n",expression_name(current_expression)); }
        ...
        draw_face(current_expression,ms); oled_show(); frame_count++; if(frame_count%40==0) printf("JR_ALIVE v=2 expression=%s demo=%d commands=%lu frames=%lu\n",expression_name(current_expression),demo_mode?1:0,(unsigned long)command_count,(unsigned long)frame_count); vTaskDelay(pdMS_TO_TICKS(80)); }
}
